...
def calc_d_eye(landmark, mode='arcface'):
    M, pose_index = estimate_norm(landmark, 112, mode)
...
def calc_error(landmark):
    ...
    M, pose_index = estimate_norm(landmark, 112, 'arcface')
    
    transformed_landmarks = []
    for i in range(5):
        point_homogeneous = np.array(landmark[i].tolist() + [1])
        transformed_point = M.dot(point_homogeneous)
        transformed_landmarks.append(transformed_point[:2])
    
    errors = []
    for i in range(5):
        error = np.sqrt(np.sum((transformed_landmarks[i] - arcface_src[i])**2))
        errors.append(error)
